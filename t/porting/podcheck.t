...
my $copy_name = File::Spec->catfile($data_dir, 'known_pod_issues-new');
my $copy_fh;
...
sub my_safer_print {    # print, with error checking for outputting to db
    ...
    if (! print $fh @lines) {
        ...
        safer_close($fh);
        safer_unlink($name);
        die "Failure writing to $name: $save_error";
    }
}
...
if ($regen) {
    foreach (sort { lc $a cmp lc $b } keys %valid_modules) {
        my_safer_print($copy_fh, $copy_name, $_, "\n");
    }
}
    ...
    SKIP: {
        ...
        if ($regen) {
            foreach my $message ( sort keys %{$problems{$filename}}) {
                ...
                print $copy_fh "$filename\t$message\t$count\n";
            }
            next;
        }
...
if ($regen) {
    ...
    rename_if_different($copy_name, $known_issues);
}
